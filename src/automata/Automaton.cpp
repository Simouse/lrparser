...
StateID Automaton::addState(const char *label,
                            util::BitSet<ActionID> *constraints) {
    auto id = static_cast<StateID>(states.size());
    states.emplace_back(id, label, transitionSetProvider->requestResource(),
                        constraints);
    return id;
}
...
String Automaton::dumpClosure(Closure const &closure) const {
    String s;
    ...
    for (auto stateID : closure) {
        ...
        auto const &constraints = formerStates[stateID].constraint;
        if (includeConstraints && constraints) {
            s += ", ";
            bool slashFlag = false;
            for (auto actionID : *constraints) {
...
Automaton Automaton::toDFA() {
    ...
    std::stack<StateID> stack;
    std::unordered_map<Closure, StateID, std::hash<Closure>,
                       decltype(closureEqualFunc)>
        closureIDMap(estimatedSize, std::hash<Closure>(), closureEqualFunc);
